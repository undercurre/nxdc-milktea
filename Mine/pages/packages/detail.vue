    ...
    <view style="padding: 30rpx 40rpx; padding-bottom: 100rpx">
      ...
      <view class="d-flex flex-column w-100">
        <view
          ...
          v-for="(coupon, index) in package.coupons"
          :key="index"
      ...
      <view class="font-size-base text-color-base">
        售卖时间：{{ package.start_at.split(" ")[0] }}~{{
          package.end_at.split(" ")[0]
        }}
      </view>
  ...
  computed: {
    couponNum() {
      return (
        ...
        this.package.coupons.reduce((acc, coupon) => acc + coupon.coupon_num, 0)
      );
    },
  },
  async onLoad(option) {
